...
get_pop <- function(annee){
  if( annee < 2017){
    dt_pop_iris <- read_xls(paste0("./src/base-ic-evol-struct-pop-",annee,".xls"),skip=5)
    dt_pop_iris_com <- read_xls(paste0("./src/base-ic-evol-struct-pop-",annee,"-com.xls"),skip=5)
    }
  else {
    dt_pop_iris <- read_xlsx(paste0("./src/base-ic-evol-struct-pop-",annee,".xlsx"),skip=5)
    dt_pop_iris_com <- read_xlsx(paste0("./src/base-ic-evol-struct-pop-",annee,"-com.xlsx"),skip=5)
    }
...
dt_pop <- lapply(2014:2018,get_pop)%>%
  bind_rows()
...
read_iris<-function(annee){
  ...
  url_zip(url)
  file<-grep(paste0("reference_IRIS_geo",annee),dir("./src"),value=T)
  if (str_detect(file,".xlsx$")){
    dt_ref<-read_xlsx(file.path("./src",file),skip=5,sheet=1)
    if(annee>=2016)
      dt_modif<-read_xlsx(file.path("./src",file),skip=5,sheet="Modifications_IRIS")%>%
        rename_all(tolower)%>%
        select(annee_modif,
               modif_iris,
               iris_ini,
    ...
    dt_ref<-read_xls(file.path("./src",file),skip=5-(annee==2013),sheet=1)
    if(annee>=2016)
      dt_modif<-read_xls(file.path("./src",file),skip=5,sheet="Modifications_IRIS")%>%
        rename_all(tolower)%>%
        select(annee_modif,
               modif_iris,
               iris_ini,
...
tab_iris<-lapply(2008:2021,read_iris)%>%
  bind_rows()
...
dt_pass_iris<-tab_iris%>%select(-dt_ref)%>%
  unnest(dt_modif)%>%
  filter(annee_geo==annee_modif)%>%
  select(-annee_modif)

shap_iris2016<-read_sf("./src/shap_iris2016/CONTOURS-IRIS.shp")%>%
  select(com=INSEE_COM,iris=CODE_IRIS)
shap_iris2017<-read_sf("./src/shap_iris2017/CONTOURS-IRIS.shp")%>%
  select(com=INSEE_COM,iris=CODE_IRIS)
shap_iris2018<-read_sf("./src/shap_iris2018/CONTOURS-IRIS.shp")%>%
  select(com=INSEE_COM,iris=CODE_IRIS)
shap_iris2019<-read_sf("./src/shap_iris2019/CONTOURS-IRIS.shp")%>%
  select(com=INSEE_COM,iris=CODE_IRIS)
shap_iris2020<-read_sf("./src/shap_iris2020/CONTOURS-IRIS.shp")%>%
  select(com=INSEE_COM,iris=CODE_IRIS)
shap_iris2021<-read_sf("./src/shap_iris2021/CONTOURS-IRIS.shp")%>%
  select(com=INSEE_COM,iris=CODE_IRIS)
...
scis_com<-readRDS("../com/dt_scis_com.rds")
pop_com<-readRDS("../com/dt_pop_com.rds")
pass_com<-readRDS("../com/dt_pass_com.rds")
...
saveRDS(dt_pass_iris,file="dt_pass_iris.rds")
